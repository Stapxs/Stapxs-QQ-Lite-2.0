...
<template>
  <div class="opt-page">
    <div v-if="Object.keys(runtimeData.loginInfo).length > 0" class="ss-card account-info">
      ...
      <svg :title="$t('base.exit')" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M160 96c17.7 0 32-14.3 32-32s-14.3-32-32-32H96C43 32 0 75 0 128V384c0 53 43 96 96 96h64c17.7 0 32-14.3 32-32s-14.3-32-32-32H96c-17.7 0-32-14.3-32-32l0-256c0-17.7 14.3-32 32-32h64zM504.5 273.4c4.8-4.5 7.5-10.8 7.5-17.4s-2.7-12.9-7.5-17.4l-144-136c-7-6.6-17.2-8.4-26-4.6s-14.5 12.5-14.5 22v72H192c-17.7 0-32 14.3-32 32l0 64c0 17.7 14.3 32 32 32H320v72c0 9.6 5.7 18.2 14.5 22s19 2 26-4.6l144-136z"/></svg>
    </div>
    <div class="ss-card">
      <header>{{ $t('option.account.config') }}</header>
    </div>
    <div class="ss-card" v-if="Object.keys(runtimeData.botInfo).length > 0">
      <header>{{ $t('option.account.bot') }}</header>
      <div class="l10n-info">
        ...
        <div>
          <span>{{ runtimeData.botInfo.app_name }}<a>{{ runtimeData.botInfo.app_version !== undefined ? runtimeData.botInfo.app_version : runtimeData.botInfo.version }}</a></span>
          <span>{{ $t('option.account.bot_tip') }}</span>
        </div>
      </div>
      <div class="bot-info">
        <div v-for="key in Object.keys(runtimeData.botInfo)" :key="'botinfo-' + key">
          <span v-if="key !== 'app_name' && key !== 'app_version' && key !== 'version'">
            <span>{{ $t('botinfo.' + key) + ': ' }}</span>
            <span v-if="(typeof runtimeData.botInfo[key]) !== 'object'">
              {{ paseBotInfo(key, runtimeData.botInfo[key]) }}
            </span>
            <span v-else
                  v-for="item in Object.keys(runtimeData.botInfo[key])"
                  v-show="(typeof runtimeData.botInfo[key][item]) !== 'object'"
                  :key="'botinfo-' + key + item">
              {{ $t('botinfo.' + item) + ': ' + paseBotInfo(item, runtimeData.botInfo[key][item]) }}
            </span>
          </span>
        </div>
      </div>
