        ...
        <div :class="'chat-info-base ' + chat.show.type">
          <div>
            ...
            <div v-if="chat.show.type === 'group'">
              <svg :title="$t('chat_chat_info_is_owner')" v-if="chat.info.group_info.gOwner === runtimeData.loginInfo.uin"
            ...
            <div class="outher">
              <span>{{ $t('chat_chat_info_birthday') }}:
                <span>
                  ...
                  ) + ` (${$t('chat_chat_info_chinese_zodiac').split('|')[chat.info.user_info.shengxiao - 1]})` }}
                </span>
              </span>
  ...
  export default defineComponent({
    ...
    methods: {
      ...
      saveTop(event: Event) {
        ...
        runtimeData.sysConfig.top_info = topList
        // 刷新 cookie
        app.config.globalProperties.$cookies.set('top', JSON.stringify(topList), '1m')
      }
    },
    watch: {
      chat: function () {
        ...
        // 修改 isTop
        if (topList !== undefined) {
          this.isTop = topList.indexOf(Number(this.chat.show.id)) >= 0
        }
      }
    }
  })
