    ...
    <div class="ss-card face-pan">
      <div class="layui-tab layui-tab-brief">
        <ul class="layui-tab-title chat-info-tab fase-pan-tab">
          ...
          <li v-if="runtimeData.stickers !== undefined">
        ...
        <div class="layui-tab-content">
          ...
          <div class="layui-tab-item">
            <div class="face-stickers">
              <img
                loading="lazy"
                v-for="(url, index) in runtimeData.stickers"
                @click="addImgFace(url)"
                :key="'stickers-' + index"
                :src="url"
              >
            <div v-show="runtimeData.stickers !== undefined && runtimeData.stickers.length <= 0" class=ss-card>
              ...
              <span>{{ $t('chat.face_pan.none').split('|')[0] }}</span>
            </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    addSpecialMsg: function (json, addText) {
  ...
  mounted () {
    // 加载漫游表情
    if (runtimeData.stickers === undefined) {
      connecter.send('get_roaming_stamp', null, 'getRoamingStamp')
    }
  }
}
</script>
